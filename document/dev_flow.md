...
- [ ] 各ページのUI作成（修正しやすいようにコンポーネントは切り分けずダミーデータも適当にハードコーディングする）。
- [ ] 各ページのUI修正
- [ ] UIデザイン決定
...
- [ ] ダミーデータを作成し`./web/app/dummy-data`にJSONで保存
- [ ] ダミーデータをアプリ上の`/web/app/dev/import`からfirestoreに登録。

### プロトタイプ作成
プロトタイプ作成TODO。必要に応じて、詳細なTODOの追加や修正あり。

- [ ] データ取得ロジックの実装（取得失敗や、データなしの場合のロジックも）。
- [ ] データ登録ロジックの実装。
- [ ] データ修正ロジックの実装。
- [ ] データ削除ロジックの実装。
- [ ] 認証ロジックの実装。
