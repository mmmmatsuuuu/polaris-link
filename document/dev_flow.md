...
### 一括登録の実装手順
- [x] `app/admin/*/bulk`のUIモック作成
- [x] UIモックのレビューと修正
- [x] 開発環境で使用するダミーのJSONデータ作成
- [x] APIの作成
- [x] 流し込みのテスト
- [x] テストをもとに修正

### 修正
- [x] 小テストの結果ページのUI修正（正解・不正解が分かりにくい。自分の回答と模範回答をわかりやすく分離）
- [x] コンテンツのtagsがうまく設定されていない問題
- [x] 授業のtagsもうまく設定されていない（bulkではフィールドがあるが、通常の編集だとフィールドがない）。
