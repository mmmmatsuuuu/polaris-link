...
- **公開状態**:
    ...
    - 公開／非公開の切り替えによって利用できるコンテンツを制御し、年度単位の利用設定は行わない。
    - 非公開状態のコンテンツは生徒画面および授業一覧に表示しない。
- **一括登録**:
    - 管理画面ごとに対象データをJSONファイルで一括登録できる（生徒/科目/単元/授業/コンテンツ）。
    - JSONフォーマット、必須項目、バリデーションルールを定義し、アップロード時に検証・エラー表示を行う。
    - 総ドキュメント数は300件以下とし、サーバー側で書き込み単位に変換して1バッチでFirestoreへ登録する。失敗時は全件ロールバックする。
    - IDは自動生成とし、重複はバリデーションで検知する。
...
- **生徒向け**:
    - ダッシュボードでは挨拶、総学習時間、直近で利用した授業（最大5件）を確認でき、科目利用状況ページへの導線を提供する。
    - 科目利用状況ページで科目→単元→授業単位の利用状況や視聴率・正答率を確認できる。
